...
# Split dataset
cluster_membersip = {0: [], 1: []}
for idx, ppi in enumerate(ppis):
    if ppi.partition == 'INTRA1':
        cluster_membersip[0].append(idx)
    elif ppi.partition == 'INTRA0':
        cluster_membersip[1].append(idx)
print(len(cluster_membersip[0]), len(cluster_membersip[1]))
...
split = Split(
    ...
    kfold = 0,
    cluster_membership = cluster_membersip
)
